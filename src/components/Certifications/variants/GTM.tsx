...
const GTMCertifications = ({
  certifications: allCerts,
  title = "Trust Badges & Certifications 🛡️",
  subtitle = "Industry-recognized credentials that validate expertise in GTM tools, analytics, and revenue operations",
}: CertificationsProps) => {
  ...
  const renderBadgeCard = (cert: Certification, index: number) => (
    <motion.div
      ...
      viewport={{ once: true }}
      className="group relative bg-white rounded-2xl border-2 border-gray-200 hover:border-emerald-400 shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden"
    >
      {/* Badge Ribbon */}
      <div className="absolute top-0 right-0 w-16 h-16 overflow-hidden">
        <div className="absolute top-2 right-[-28px] w-28 bg-emerald-500 text-white text-[10px] font-bold uppercase tracking-wider py-1 text-center transform rotate-45">
          Verified
        </div>
      </div>
      ...
      {cert.cert_url ? (
        <Link
          ...
          className="block"
        >
          <div className="h-40 w-full flex items-center justify-center cursor-pointer border-b border-gray-100 bg-gradient-to-br from-gray-50 to-white hover:from-emerald-50/50 hover:to-white transition-colors p-6">
            <Image
...
export default GTMCertifications;
