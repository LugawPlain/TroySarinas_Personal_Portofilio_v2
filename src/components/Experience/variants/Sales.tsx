...
import { Trophy, Target, DollarSign, TrendingUp, Users, MapPin, Briefcase } from "lucide-react";
...
const SalesExperience = ({ initialExperience }: ExperienceProps) => {
  return (
    <div
      id="experience"
      className="px-4 sm:px-8 font-inter flex relative flex-col space-y-12 justify-center overflow-x-clip items-center py-16 bg-gradient-to-b from-white to-gray-50/50"
    >
      {/* Section Header */}
      <div className="text-center mb-8">
        ...
        <h1 className="text-center text-3xl sm:text-4xl xl:text-5xl font-bold mb-4 text-gray-900 font-fraunces">
          Quota Attainment Timeline
        </h1>
        <p className="text-gray-600 max-w-2xl mx-auto text-lg">
          A proven track record of exceeding sales targets and driving revenue growth across multiple industries.
        </p>
      </div>

      {/* Timeline Line */}
      <div className="relative w-full max-w-[80rem]">
        <div className="absolute left-4 sm:left-8 xl:left-1/2 top-0 bottom-0 w-0.5 bg-gradient-to-b from-red-600 via-red-400 to-red-200 hidden sm:block" />
        ...
        {initialExperience.map((exp, index) => {
          ...
          return (
            ...
            >
              {/* Timeline Dot */}
              <div className="absolute left-4 sm:left-8 xl:left-1/2 w-4 h-4 bg-red-600 rounded-full border-4 border-white shadow-lg hidden sm:block transform -translate-x-1/2 mt-8 z-10" />

              {/* Content Card */}
              <div className={contentClass}>
                <div className="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden hover:shadow-xl transition-shadow duration-300">
                  {/* Top Bar with Logo and Company Info */}
                  <div className={(exp.logo_bg_color || "bg-gray-900") + " p-6 flex items-center gap-4"}>
                  ...
                  {/* Quota Metrics Banner - Big Numbers */}
                  {metricHighlights.length > 0 && (
                    <div className="bg-gradient-to-r from-red-50 to-red-100/50 border-b border-red-100 p-6">
                      <div className="flex items-center gap-2 mb-4">
                        <Target className="w-4 h-4 text-red-600" />
                        <span className="text-xs font-bold text-red-600 uppercase tracking-wider">Quota Performance</span>
                      </div>
                      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                        {metricHighlights.slice(0, 4).map((highlight, idx) => (
                          <div key={idx} className="text-center bg-white rounded-xl p-4 shadow-sm border border-red-100">
                  ...
                  <div className="p-6">
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-6">
                      <div className="bg-red-50 rounded-xl p-4">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <span className="text-xs font-bold text-gray-500 uppercase">Territory</span>
                        </div>
                        <p className="text-sm font-semibold text-gray-800">West Coast Enterprise</p>
                      </div>
                      <div className="bg-gray-50 rounded-xl p-4">
                        <div className="flex items-center gap-2 mb-2">
                          <Users className="w-4 h-4 text-gray-600" />
                          <span className="text-xs font-bold text-gray-500 uppercase">Vertical</span>
                        </div>
                        <p className="text-sm font-semibold text-gray-800">SaaS / Technology</p>
                      </div>
                    </div>
                    ...
                    {exp.highlights.filter(h => !parseMetric(h.title)).length > 0 && (
                      <div className="mb-6">
                        <h3 className="text-sm font-bold text-gray-900 uppercase tracking-wider mb-3">Key Achievements</h3>
