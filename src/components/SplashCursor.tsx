...
export default function SplashCursor({
  ...
  DENSITY_DISSIPATION = 4.5, // Increased for faster dissipation
  VELOCITY_DISSIPATION = 3, // Increased for faster dissipation
  PRESSURE = 0.08, // Reduced pressure
  PRESSURE_ITERATIONS = 16, // Reduced iterations
  CURL = 2, // Reduced curl
  SPLAT_RADIUS = 0.15, // Reduced radius
  SPLAT_FORCE = 4000, // Reduced force
  SHADING = false, // Disabled shading for performance
  COLOR_UPDATE_SPEED = 8, // Reduced color update speed
  BACK_COLOR = { r: 0.5, g: 0, b: 0 },
  TRANSPARENT = true,
}: SplashCursorProps) {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const { isCursorEffectEnabled } = useCursor();

  useEffect(() => {
    if (!isCursorEffectEnabled) return;
  ...
  if (!isCursorEffectEnabled) return null;
