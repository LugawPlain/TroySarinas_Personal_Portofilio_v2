...
const SalesHeader = () => {
  ...
  return (
    <>
      <div
        className={`lg:px-16 Header top-0 bg-white z-50 backdrop-blur-2xl sticky flex items-center justify-between px-4 h-20 border-b-2 border-red-600/10 transition-transform duration-300 ${
          isVisible ? "translate-y-0" : "-translate-y-full"
        }`}
      >
        <div className="flex items-center gap-3">
          <NameTitle className="text-nowrap cursor-pointer" />
          <div className="hidden sm:flex items-center gap-1 bg-red-50 text-red-700 px-2 py-1 rounded-full text-xs font-bold">
        ...
        <div className="flex items-center gap-6">
            ...
            <ul className="flex gap-6 text-lg items-center">
              {navLinks.map((link) => (
                <li key={link.name}>
                  <a
                    ...
                    className="hover:text-red-600 font-inter transition-colors font-semibold"
                  >
                    {link.name}
                  </a>
                </li>
              ...
              <li>
                <Button
                  ...
                  className="text-md font-bold px-4 py-2 bg-red-600 text-white uppercase tracking-tight shadow-lg hover:bg-red-700"
                >
                  Let&apos;s Close Deals
                </Button>
              </li>
          ...
          <div className="relative">
            <button
              ...
              aria-label="Settings"
            >
              <RiSettings5Fill className="text-red-600" size={30} />
            </button>
                ...
                <div className="p-2">
                  ...
                  <div className="flex items-center justify-between py-2 px-3 border-t border-gray-100">
                    ...
                    <button
                      ...
                      className={`relative cursor-pointer inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                        isCursorEffectEnabled ? "bg-red-600" : "bg-gray-200"
                      }`}
                    >
                      <span
                        className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
      ...
      <div
        className={`xl:hidden fixed top-20 left-0 right-0 bg-white border-b-2 border-red-600/10 z-40 transition-all duration-300 overflow-hidden ${
      ...
      >
        <nav>
          <ul className="flex flex-col divide-y-[0.5px] divide-gray-200">
            {navLinks.map((link) => (
              <li key={link.name} className="active:bg-red-600">
                <a
                  href={
                    link.href.startsWith("#")
                      ? (rolePrefix || "") + link.href
                  ...
                  onClick={(e) => handleSmoothScroll(e, link.href)}
                  className="block text-lg hover:text-red-600 px-4 py-3 active:text-white font-inter transition-colors font-semibold"
                >
                  {link.name}
                </a>
              </li>
            ...
            <li className="border-t border-gray-200">
              <Button
                ...
                className="text-md font-bold w-full bg-red-600 text-white uppercase tracking-tight shadow-lg"
              >
                Let&apos;s Close Deals
              </Button>
            </li>
