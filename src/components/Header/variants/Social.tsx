    ...
    <>
      <motion.div
        ...
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
          isScrolled ? "bg-gray-950/90 backdrop-blur-xl border-b border-gray-800" : "bg-transparent"
        }`}
      >
        <div className="flex items-center justify-between px-4 sm:px-8 h-16">
          <a href={rolePrefix || "/"} className="flex items-center gap-2 text-white font-bold text-lg">
              ...
              <ul className="flex gap-1 items-center">
                  ...
                  return (
                    <li key={link.name}>
                      ...
                      >
                        {link.name}
                        {isActive && (
                          <motion.div layoutId="socialActive" className="absolute bottom-0 left-2 right-2 h-px bg-pink-400" />
                        )}
                      </a>
                    </li>
                  );
                ...
                <li className="ml-4">
                  <Button
                    ...
                    className="text-xs font-semibold px-5 py-2 bg-pink-600 hover:bg-pink-500 text-white rounded-full transition-all"
                  >
        ...
        {isMobileMenuOpen && (
          <>
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="fixed inset-0 bg-gray-950/95 backdrop-blur-xl z-40 xl:hidden"
            >
                ...
                <nav className="flex flex-col items-center gap-6">
                  {navLinks.map((link, index) => (
                    <motion.a
                      ...
                      className="text-2xl font-medium text-gray-300 hover:text-pink-400"
                    >
                      {link.name}
                    </motion.a>
                  ))}
                  <Button
                    ...
                    className="mt-8 px-8 py-3 bg-pink-600 hover:bg-pink-500 text-white rounded-full"
                  >
                    Get in Touch
                  </Button>
                </nav>
